...
//
// Requires: google_fonts: ^6.2.0   (flutter pub add google_fonts)
//
// Usage:
//   MaterialApp(theme: SpotlessTheme.light(), ...)
//   // or with tweaks:
//   MaterialApp(theme: SpotlessTheme.light(const SpotlessThemeConfig(
//     headingFont: 'Poppins', radius: 16, heroStyle: HeroStyle.primary)))
//
...
class SpotlessThemeConfig {
  const SpotlessThemeConfig({
    ...
    this.headingFont = 'Space Grotesk',
    this.bodyFont = 'Public Sans',
    this.radius = 22,
  });
  ...
  /// Any Google Font name: 'Space Grotesk', 'Poppins', 'Plus Jakarta Sans',
  /// 'Outfit', 'Fraunces', 'DM Serif Display'.
  final String headingFont;

  /// 'Public Sans', 'DM Sans', 'Manrope', 'Nunito Sans', 'Work Sans'.
...
class SpotlessTokens extends ThemeExtension<SpotlessTokens> {
  ...
  TextStyle heading(double size, {Color? color, FontWeight weight = FontWeight.w700}) =>
      GoogleFonts.getFont(headingFont, fontSize: size, fontWeight: weight, color: color, letterSpacing: -0.02 * size, height: 1.15);
...
abstract final class SpotlessTheme {
  static ThemeData light([SpotlessThemeConfig c = const SpotlessThemeConfig()]) {
    ...
    final body = GoogleFonts.getTextTheme(c.bodyFont).apply(bodyColor: SpotlessColors.ink, displayColor: SpotlessColors.ink);
    TextStyle h(double size) => GoogleFonts.getFont(c.headingFont,
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(mainAxisSize: MainAxisSize.min, children: [
        ...
        Text(label, style: Theme.of(context).textTheme.labelMedium?.copyWith(color: fg, fontSize: 12.5)),
      ]),
    );
  }
}
